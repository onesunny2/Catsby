...
/* 📌 작업 순서
 ...
 < 2페이즈 > 탭바컨트롤러☑️ - 오늘의영화(최근검색어X)☑️ - 닉네임 수정화면☑️ - 영화 좋아요 기능☑️ -  오늘의영화(최근검색어O)☑️ - 검색결과화면☑️ - 영화상세화면☑️ - 설정화면☑️ - 프로필 수정 모달☑️
 < 3페이즈 >
 ...
 10. 메인화면에서 무비박스 갯수 달라지는거 탭해서 설정화면가도 반영되는지 확인 필요 ☑️
 11. 상세화면에서 돌아왔을 때 무비박스 갯수 반영되도록 ☑️
 12. 파일 경로같이 언제든 값이 없을 것 같은 부분은 옵셔널로 교체하기
 13. 전체적으로 좋아요 값 전달하는 부분 수정하기
 
 ⚙️< 의문스러운, 질문하고싶은 부분 >
 1. 탭바가 있을 때 오늘의 영화 하단에 collectionView의 높이를 잡는법
 - layoutSubview로 해결
 2. 왜... 왜 컬렉션뷰 시작점이 밀리는걸까요...
 3. contentInset, SectionInset 근데 누구는 적용되고 적용 안되고...
 4. 동적 cell 사이즈를 구현하면서 너무 많이 뜨는 보라색 경고.. 원인을 잘 모르겠다 ㅠㅠ 시간 없어서 우선 넘기지만 끝나도 해결해보도록 노력해보자
